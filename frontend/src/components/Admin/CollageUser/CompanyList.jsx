...
const CompanyList = () => {
    ...
    const fetchCompanies = async () => {
      try {
        ...
        if (response.ok) {
          ...
          const companiesData = responseData.admins.map((company, index) => ({
            ...company,
  ...
  const handleConfirmDelete = async () => {
    try {
      const response = await adminService.getDeleteAcademic(deletedCompanyId);
  ...
  const handleUpdateCompanyList = async () => {
    try {
      const response = await adminService.getAdminPhotoById(
        page,
        companiesPerPage
      );
              ...
              {companies.map((company, index) => (
                <TableRow key={company.acadamic_id}>
                  ...
                  <TableCell>
                    <ActionsWrapper>
                      <IconButton
                        onClick={() => handleDelete(company.acadamic_id)}
                      >
                        <DeleteIcon />
                      </IconButton>
                    </ActionsWrapper>
